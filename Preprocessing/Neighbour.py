...
def find_neig(df):
    ...
    for i, grp in df.groupby(["Canal","Time"]):
        if grp.shape[0] > 1:
            ...
            df.loc[(df["Canal"]==i[0]) and (df["Time"]==i[1]),"Dist_Closest"] = closest
            df.loc[(df["Canal"]==i[0]) and (df["Time"]==i[1]), "Dist_derecha"] = di_d
            df.loc[(df["Canal"]==i[0]) and (df["Time"]==i[1]), "Dist_izquierda"] = di_i
            df.loc[(df["Canal"]==i[0]) and (df["Time"]==i[1]), "Vecino_der"] = vec_d
            df.loc[(df["Canal"]==i[0]) and (df["Time"]==i[1]), "Vecino_izq"] = vec_i       
    return df
